...
async function approveApplication(id) {
  ...
  if (result.emailSent) {
    ...
    alert("Approved — copy the access code and email it to the pharmacy (SMTP not configured).");
  }
...
async function addPharmacy() {
  const businessName = prompt("Pharmacy / business name:");
...
async function sendComplianceToPharmacy(id) {
  if (!confirm("Email the compliance pack (NDA + gummy pack + TM certificate) to this pharmacy?")) return;

...
const { catalogForPortal, getCatalogBySku } = require("./wholesale-catalog");
...
function buildHumidityRows(pricing) {
  ...
  return [
    enrichItem(
      {
        sku: "HP-SINGLE",
        name: "2-Way Humidity Pack 62% — single (retail from $1.60)",
...
function buildOrderSheet(pricing) {
  ...
  for (const category of catalogForPortal()) {
...
function catalogQtyFromSheet(rawCatalog) {
  ...
  for (const [sku, qty] of Object.entries(rawCatalog)) {
    ...
    if (getCatalogBySku()[sku]) out[sku] = n;
  }
  return out;
}

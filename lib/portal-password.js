...
function verifyTokenFields(token, pharmacy) {
  if (!pharmacy?.passwordTokenHash || !pharmacy?.passwordTokenExpiresAt) return false;
  if (pharmacy.passwordTokenExpiresAt < Date.now()) return false;
  try {
    const attempt = hashToken(token);
    const stored = Buffer.from(pharmacy.passwordTokenHash, "hex");
...
function clearTokenFields(pharmacy) {
  pharmacy.passwordTokenHash = null;
  pharmacy.passwordTokenExpiresAt = null;
  pharmacy.passwordTokenPurpose = null;
}

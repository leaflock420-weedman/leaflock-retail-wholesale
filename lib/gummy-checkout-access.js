...
const { findByCheckoutKey } = require("./pharmacy-store");
...
function resolveCheckoutAccess(req) {
  ...
  const pharmacy = findByCheckoutKey(key);
  if (pharmacy) {
    return { ok: true, source: "stockist", pharmacy };
  }

  const global = globalAccessKey();
  if (global && key === global) {
    return { ok: true, source: "campaign", pharmacy: null };
  }
...
function requireGummyCheckoutAccess(req, res, next) {
  ...
  req.gummyCheckoutPharmacy = access.pharmacy || null;
  next();
}

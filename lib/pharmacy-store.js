...
function checkoutUrlForPharmacy(pharmacy) {
  if (!pharmacy?.checkoutAccessKey) return null;
  return `${siteUrl()}/gummy-checkout.html?key=${encodeURIComponent(pharmacy.checkoutAccessKey)}`;
}
...
const PRESERVED_STOCKISTS = [
  {
    ...
    email: "wholesale@leaflock.com.au",
    status: "active",
    accessCodeHash:
...
function isDemoPharmacy(pharmacy) {
  if (!pharmacy) return false;
  return (
    pharmacy.id === DEMO_PHARMACY_ID ||
    normalizeEmail(pharmacy.email) === normalizeEmail(DEMO_EMAIL)
  );
}
...
function ensureCheckoutKeys(data) {
  ...
  for (const pharmacy of data.pharmacies) {
    ...
    if (!pharmacy.checkoutAccessKey) {
      pharmacy.checkoutAccessKey = generateCheckoutAccessKey();
      changed = true;
    }
  }
  return changed;
...
function ensurePreservedStockists(data) {
  ...
  for (const preserved of PRESERVED_STOCKISTS) {
    let existing = data.pharmacies.find((p) => p.id === preserved.id);
    if (!existing) {
      existing = data.pharmacies.find(
        (p) =>
          !isDemoPharmacy(p) &&
    ...
    if (isDemoPharmacy(existing)) continue;
...
function ensureTestPharmacy(data) {
  const template = buildDemoPharmacy();
  let demo = data.pharmacies.find(isDemoPharmacy);
  if (demo) {
    demo.id = DEMO_PHARMACY_ID;
...
function loadPharmacies() {
  try {
    ...
    const demo = data.pharmacies.find(isDemoPharmacy);
    ...
    if (changed || JSON.stringify(data) !== before) savePharmacies(data);
    return data;
  } catch (err) {
...
function publicPharmacy(pharmacy) {
  return {
    ...
    passwordSet: Boolean(pharmacy.passwordHash),
    hasCheckoutKey: Boolean(pharmacy.checkoutAccessKey),
  };
}

function adminPharmacyView(pharmacy) {
  return {
    ...publicPharmacy(pharmacy),
    checkoutLink: checkoutUrlForPharmacy(pharmacy),
  };
}

function isActivePharmacy(pharmacy) {
  return pharmacy && pharmacy.status === "active";
}
...
function findByEmail(email) {
  ...
  if (normalized === normalizeEmail(DEMO_EMAIL)) {
    ...
    const stored = data.pharmacies.find((p) => p.status === "active" && isDemoPharmacy(p));
    return stored || buildDemoPharmacy();
  }
  const data = loadPharmacies();
  return data.pharmacies.find((p) => p.status === "active" && normalizeEmail(p.email) === normalized) || null;
}

function findByPasswordToken(token) {
  if (!token) return null;
  const data = loadPharmacies();
  return data.pharmacies.find((p) => p.status === "active" && verifyTokenFields(token, p)) || null;
}
...
function regenerateCheckoutAccessKey(pharmacyId) {
  const pharmacies = loadPharmacies();
  const pharmacy = pharmacies.pharmacies.find((p) => p.id === pharmacyId);
  if (!pharmacy) return null;
  pharmacy.checkoutAccessKey = generateCheckoutAccessKey();
  savePharmacies(pharmacies);
  return { pharmacy: publicPharmacy(pharmacy), checkoutLink: checkoutUrlForPharmacy(pharmacy) };
}
...
function issuePasswordSetupToken(pharmacyId, purpose = "setup") {
  const pharmacies = loadPharmacies();
  const pharmacy = pharmacies.pharmacies.find((p) => p.id === pharmacyId && p.status === "active");
  if (!pharmacy) return null;
  const token = generateToken();
  Object.assign(pharmacy, createTokenFields(token, purpose));
  savePharmacies(pharmacies);
  return { pharmacy: publicPharmacy(pharmacy), setupToken: token };
}

function setPharmacyPassword(pharmacyId, password) {
  const message = validatePassword(password);
  if (message) return { error: message };
  const pharmacies = loadPharmacies();
  const pharmacy = pharmacies.pharmacies.find((p) => p.id === pharmacyId && p.status === "active");
  if (!pharmacy) return null;
  pharmacy.passwordHash = hashPassword(password);
  pharmacy.passwordSetAt = Date.now();
  clearTokenFields(pharmacy);
  savePharmacies(pharmacies);
  return publicPharmacy(pharmacy);
}
...
function setPasswordWithToken(token, password) {
  ...
  if (message) return { error: message };
  const pharmacies = loadPharmacies();
  const pharmacy = pharmacies.pharmacies.find((p) => p.status === "active" && verifyTokenFields(token, p));
  if (!pharmacy) return { error: "invalid_or_expired_token" };
  pharmacy.passwordHash = hashPassword(password);
  pharmacy.passwordSetAt = Date.now();
  clearTokenFields(pharmacy);
  savePharmacies(pharmacies);
...
function changePharmacyPassword(pharmacyId, currentPassword, newPassword) {
  const message = validatePassword(newPassword);
  if (message) return { error: message };
  const pharmacies = loadPharmacies();
  const pharmacy = pharmacies.pharmacies.find((p) => p.id === pharmacyId && p.status === "active");
  if (!pharmacy) return null;
  if (!verifyPassword(currentPassword, pharmacy.passwordHash)) {
    return { error: "incorrect_password" };
  }
  pharmacy.passwordHash = hashPassword(newPassword);
  pharmacy.passwordSetAt = Date.now();
  savePharmacies(pharmacies);
  return publicPharmacy(pharmacy);
}

function deletePharmacyAccount(pharmacyId, password) {
  if (pharmacyId === DEMO_PHARMACY_ID) return { error: "demo_account" };
  const pharmacies = loadPharmacies();
  const pharmacy = pharmacies.pharmacies.find((p) => p.id === pharmacyId && p.status === "active");
  if (!pharmacy) return null;
  if (!verifyPassword(password, pharmacy.passwordHash)) {
    return { error: "incorrect_password" };
  }
  pharmacy.status = "deleted";
  pharmacy.deletedAt = Date.now();
  pharmacy.passwordHash = null;
  clearTokenFields(pharmacy);
  pharmacy.accessCodeHash = null;
  savePharmacies(pharmacies);
...
function sendPasswordReset(pharmacyId) {
  return issuePasswordSetupToken(pharmacyId, "reset");
}

...
function ensureTestPharmacy(data) {
  ...
  const test = data.pharmacies.find((p) => p.businessName === "LeafLock Test Retail Store");
  if (test) {
    test.status = "active";
  ...
  console.log(`[retail] Test retail store ready — access code: ${seedCode}`);
  return data;
}
...
function loadPharmacies() {
  ...
  if (test) {
    ...
    if (test.status !== "active" || !verifyAccessCode(code, test.accessCodeHash)) {
      ...
      console.log("[retail] Test retail store access code updated");
    }
  }

...
const LEGACY_DEMO_NAMES = new Set([DEMO_BUSINESS_NAME, "LeafLock Test Retail Stockist", "LeafLock Test Retail"]);
...
function ensureTestPharmacy(data) {
  ...
  if (test) {
    ...
    test.passwordHash = hashPassword(demoPortalPassword());
    test.accessCodeHash = null;
    clearTokenFields(test);
    demoPharmacyCache = { ...test };
    return data;
  }
  data.pharmacies.unshift({ ...demo });
...
function loadPharmacies() {
  try {
    ...
    if (test) {
      if (test.status !== "active" || !test.passwordHash) {
        test.status = "active";
        test.passwordHash = hashPassword(demoPortalPassword());
        test.accessCodeHash = null;
...
function findByEmail(email) {
  ...
  if (normalized === normalizeEmail("demo@leaflock.com.au")) {

...
function reconcilePreservedRetailData() {
  ...
  if (syncApprovedApplicationsWithStockists()) changed = true;
  if (changed) {
...
function initializeRetailData() {
  ...
  const carryover = mergeCarryoverSnapshotOnStartup();
  const migrated = migrateEphemeralDataIfNeeded();
  const backed = backupAllProtectedFiles();
  const restore = restoreAndMergeProtectedData();
  let changed = carryover.changed || migrated.changed || restore.changed;
  const results = [...carryover.results, ...migrated.results, ...restore.results];
...
/** Only adds stockists that are completely missing — never edits existing accounts. */
function ensurePreservedStockists(data) {
  let changed = false;
  for (const preserved of PRESERVED_STOCKISTS) {
    const existsById = data.retailStockists.some((p) => p.id === preserved.id);
    const existsByEmail = data.retailStockists.some(
      (p) =>
        !isDemoStockist(p) &&
        normalizeEmail(p.email) === normalizeEmail(preserved.email),
    );
    if (existsById || existsByEmail) continue;
    data.retailStockists.push({
      ...preserved,
      checkoutAccessKey: preserved.checkoutAccessKey || generateCheckoutAccessKey(),
    });
    changed = true;
    console.log(`[retail] Added missing stockist: ${preserved.businessName}`);
  }
  return changed;
}
...
const STOCKISTS_LOCK_FILE = path.join(DATA_DIR, "retail-stockists.lock");

function withStockistsLock(fn) {
  ensureDataDir();
  let fd;
  try {
    for (let attempt = 0; attempt < 80; attempt++) {
      try {
        fd = fs.openSync(STOCKISTS_LOCK_FILE, "wx");
        break;
      } catch {
        const spinUntil = Date.now() + 25;
        while (Date.now() < spinUntil) {
          /* wait for concurrent write */
        }
      }
    }
    if (fd === undefined) throw new Error("Could not lock retail stockist data");
    return fn();
  } finally {
    if (fd !== undefined) {
      try {
        fs.closeSync(fd);
        fs.unlinkSync(STOCKISTS_LOCK_FILE);
      } catch {
        /* lock released or stale */
      }
    }
  }
}

function getRetailStockists() {
  const data = readRetailStockistsFile();
  if (!Array.isArray(data.retailStockists)) data.retailStockists = [];
  return data;
}
...
function saveRetailStockists(data) {
  return writeJson(RETAIL_STOCKISTS_FILE, data);
}

function saveRetailStockistsChecked(data) {
  if (!saveRetailStockists(data)) {
    throw new Error("Could not save retail stockist data");
  }
}

function persistStockistPassword(stockistId, password) {
  return withStockistsLock(() => {
    const data = getRetailStockists();
    const retailStockist = data.retailStockists.find((p) => p.id === stockistId && p.status === "active");
    if (!retailStockist) return { error: "account_not_found" };
    retailStockist.passwordHash = hashPassword(password);
    retailStockist.passwordSetAt = Date.now();
    retailStockist.mustChangePassword = false;
    retailStockist.accessCodeHash = null;
    clearTokenFields(retailStockist);
    saveRetailStockistsChecked(data);
    const saved = getRetailStockists().retailStockists.find((p) => p.id === stockistId);
    if (!hasPortalPassword(saved) || !verifyPassword(password, saved.passwordHash)) {
      console.error("[retail] Password save verification failed for", stockistId);
...
function findByPasswordToken(token) {
  const candidates = passwordTokenCandidates(token);
  if (!candidates.length) return null;
  const data = getRetailStockists();
  for (const candidate of candidates) {
    const match = data.retailStockists.find(
      (p) => p.status === "active" && verifyTokenFields(candidate, p),
    );
...
function publicApplication(application) {
  ...
  return { ...safe, passwordReady: Boolean(passwordHash) };
}
...
function issuePasswordSetupToken(retailStockistId, purpose = "setup") {
  return withStockistsLock(() => {
    const stockists = getRetailStockists();
    const retailStockist = stockists.retailStockists.find((p) => p.id === retailStockistId && p.status === "active");
    if (!retailStockist) return null;
    const token = generateToken();
    const setupCode = generateSetupCode();
    Object.assign(retailStockist, createTokenFields(token, purpose, setupCode));
    saveRetailStockistsChecked(stockists);
    return {
...
function findStockistForPasswordSetup({ token, email, code }) {
  const stockists = getRetailStockists().retailStockists.filter((p) => p.status === "active");
  const candidates = passwordTokenCandidates(token);
  if (candidates.length) {
    for (const candidate of candidates) {
      const match = stockists.find((p) => verifyTokenFields(candidate, p));
      if (match) return match;
    }
    return null;
  }
  const normalizedEmail = normalizeEmail(email);
  const normalizedCode = normalizeSetupCode(code);
  if (!normalizedEmail || !normalizedCode) return null;
  if (verifyMasterResetCode(code)) {
    return stockists.find((p) => normalizeEmail(p.email) === normalizedEmail) || null;
  }
  return (
    stockists.find(
      (p) => normalizeEmail(p.email) === normalizedEmail && verifySetupCode(normalizedCode, p),
    ) || null
  );
}

function setPasswordForStockist({ token, email, code, password }) {
  const cleanedPassword = String(password || "");
  const message = validatePassword(cleanedPassword);
  if (message) return { error: message };
  const match = findStockistForPasswordSetup({ token, email, code });
  if (!match) return { error: "invalid_or_expired_token" };
  return persistStockistPassword(match.id, cleanedPassword);
}

function adminSetStockistPassword(retailStockistId, password) {
  const message = validatePassword(password);
  if (message) return { error: message };
  return persistStockistPassword(retailStockistId, String(password));
}
...
function setPasswordWithToken(token, password) {
  return setPasswordForStockist({ token, password });
}
...
function findStockistForApproval(email) {
  const normalized = normalizeEmail(email);
  if (!normalized) return null;
  const matches = loadRetailStockists().retailStockists.filter(
    (p) => p.status !== "deleted" && normalizeEmail(p.email) === normalized,
  );
  return matches.find((p) => p.status === "active") || matches[0] || null;
}

function portalAccessStatusForEmail(email) {
  const normalized = normalizeEmail(email);
  if (!normalized) {
    return { pending: false, approved: false, canLogin: false };
  }
  const stockist = loadRetailStockists().retailStockists.find(
    (p) => p.status === "active" && normalizeEmail(p.email) === normalized,
  );
  const applications = loadApplications().applications;
  const pendingApp = applications.find(
    (a) => a.status === "pending" && normalizeEmail(a.email) === normalized,
  );
  const approvedApp = applications.find(
    (a) => a.status === "approved" && normalizeEmail(a.email) === normalized,
  );
  return {
    pending: Boolean(pendingApp),
    approved: Boolean(approvedApp) || Boolean(stockist),
    canLogin: Boolean(stockist?.passwordHash?.salt && stockist?.passwordHash?.hash),
  };
}

function syncApprovedApplicationsWithStockists() {
  const apps = loadApplications();
  const stockists = loadRetailStockists();
  let changed = false;
  for (const app of apps.applications) {
    ...
    const email = normalizeEmail(app.email);
    let stockist =
      stockists.retailStockists.find((p) => p.id === app.retailStockistId) ||
      stockists.retailStockists.find(
        (p) => p.status !== "deleted" && normalizeEmail(p.email) === email,
      );
    if (!stockist) continue;
    ...
    if (app.retailStockistId !== stockist.id) {
      app.retailStockistId = stockist.id;
      changed = true;
    }
  }
...
function approveApplication(applicationId) {
  ...
  if (app.status !== "pending") {
    return { error: "already_processed", application: publicApplication(app) };
  }

  const passwordReady = Boolean(app.passwordHash?.salt && app.passwordHash?.hash);
  const needsSetupLink = !passwordReady;
  let setupToken = null;
  let setupCode = null;
  if (needsSetupLink) {
  ...
  let retailStockist = findStockistForApproval(appEmail);

  if (retailStockist) {
    retailStockist.status = "active";
    retailStockist.businessName = app.businessName || retailStockist.businessName;
    ...
    retailStockist.approvedAt = Date.now();
    retailStockist.applicationId = app.id;
    if (passwordReady) {
      retailStockist.passwordHash = app.passwordHash;
      retailStockist.passwordSetAt = Date.now();
      retailStockist.accessCodeHash = null;
      clearTokenFields(retailStockist);
    } else if (!hasPortalPassword(retailStockist)) {
      Object.assign(retailStockist, createTokenFields(setupToken, "setup", setupCode));
    }
  } else {
    retailStockist = {
      id: newId("retail"),
      ...
      email: appEmail,
      status: "active",
      passwordHash: passwordReady ? app.passwordHash : null,
      passwordSetAt: passwordReady ? Date.now() : null,
      accessCodeHash: null,
      abn: app.abn,
      ...
      ...(needsSetupLink ? createTokenFields(setupToken, "setup", setupCode) : {}),
    };
  ...
  return {
    ...
    setupToken,
    setupCode,
    passwordReady: hasPortalPassword(retailStockist),
    checkoutLink: checkoutUrlForStockist(retailStockist),
  };
}
...
function createRetailStockist(payload) {
  ...
  const retailStockist = {
    ...
    loginCount: 0,
    checkoutAccessKey: generateCheckoutAccessKey(),
    ...createTokenFields(setupToken, "setup", setupCode),
  };
  stockists.retailStockists.unshift(retailStockist);
  saveRetailStockists(stockists);
  return {
...
function issueTemporaryPasswordReset(retailStockistId) {
  return withStockistsLock(() => {
    const stockists = getRetailStockists();
    const retailStockist = stockists.retailStockists.find(
      (p) => p.id === retailStockistId && p.status === "active",
    );
    if (!retailStockist) return null;
    const temporaryPassword = generateTemporaryPassword();
    retailStockist.passwordHash = hashPassword(temporaryPassword);
    retailStockist.mustChangePassword = true;
    retailStockist.passwordSetAt = Date.now();
    retailStockist.accessCodeHash = null;
    clearTokenFields(retailStockist);
    saveRetailStockistsChecked(stockists);
...
function completeRequiredPasswordChange(retailStockistId, newPassword) {
  const message = validatePassword(newPassword);
  if (message) return { error: message };
  return withStockistsLock(() => {
    const data = getRetailStockists();
    const retailStockist = data.retailStockists.find(
      (p) => p.id === retailStockistId && p.status === "active",
    );
    if (!retailStockist) return { error: "account_not_found" };
    if (!retailStockist.mustChangePassword) return { error: "not_required" };
    retailStockist.passwordHash = hashPassword(newPassword);
    retailStockist.mustChangePassword = false;
    retailStockist.passwordSetAt = Date.now();
    clearTokenFields(retailStockist);
    saveRetailStockistsChecked(data);
    const saved = getRetailStockists().retailStockists.find((p) => p.id === retailStockistId);
    if (!verifyPassword(newPassword, saved.passwordHash)) {
      return { error: "save_failed" };
    }
    return { retailStockist: publicRetailStockist(retailStockist) };
  });
...
function sendPasswordReset(retailStockistId) {
  return issueTemporaryPasswordReset(retailStockistId);
}

function adminRemoveRetailStockist(retailStockistId) {
  if (retailStockistId === DEMO_STOCKIST_ID) return { error: "demo_account" };
  const stockists = loadRetailStockists();
  const retailStockist = stockists.retailStockists.find((p) => p.id === retailStockistId);
  if (!retailStockist) return null;

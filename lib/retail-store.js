...
function hasPortalPassword(stockist) {
  return hasStoredPassword(stockist?.passwordHash);
}
...
function publicRetailStockist(retailStockist) {
  return {
    ...
    passwordSet: hasPortalPassword(retailStockist),
...
function publicApplication(application) {
  ...
  return { ...safe, passwordReady: hasStoredPassword(passwordHash) };
}
...
function portalAccessStatusForEmail(email) {
  ...
  return {
    ...
    canLogin: hasStoredPassword(stockist?.passwordHash),
  };
}
...
function approveApplication(applicationId) {
  ...
  const passwordReady = hasStoredPassword(app.passwordHash);
  ...
  if (retailStockist) {
    ...
    retailStockist = {
      ...
      ...(needsSetupLink ? createTokenFields(setupToken, "setup") : {}),
    };
    if (needsSetupLink) retailStockist.passwordSetupCodeHash = null;

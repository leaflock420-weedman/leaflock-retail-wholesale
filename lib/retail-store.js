  ...
  for (const preserved of PRESERVED_STOCKISTS) {
    ...
    if (hasPortalPassword(existing)) {
      for (const [key, value] of Object.entries(preserved)) {
        if (PRESERVED_AUTH_FIELDS.includes(key)) continue;
        if (existing[key] == null || existing[key] === "") existing[key] = value;
      }
      if (!existing.checkoutAccessKey && preserved.checkoutAccessKey) {
        existing.checkoutAccessKey = preserved.checkoutAccessKey;
      }
    } else {
...
function findByPasswordToken(token) {
  ...
  const data = getRetailStockists();
  return data.retailStockists.find((p) => p.status === "active" && verifyTokenFields(cleaned, p)) || null;
}
...
function findStockistForPasswordSetup({ token, email, code }) {
  ...
  if (cleanedToken) {
    return stockists.find((p) => verifyTokenFields(cleanedToken, p)) || null;
  }

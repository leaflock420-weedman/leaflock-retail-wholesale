...
function wholesaleEmail() {
  return process.env.WHOLESALE_EMAIL_TO || process.env.ANALYTICS_EMAIL_TO || "med@leaflock.com.au";
}

function fromEmail() {
  return process.env.ANALYTICS_EMAIL_FROM || process.env.SMTP_USER || "med@leaflock.com.au";
}
...
async function notifyAdminNewApplication(app) {
  const portalUrl = process.env.SITE_URL || "https://med.leaflock.com.au";
  const html = `
    <div style="font-family:Inter,Arial,sans-serif;color:#17211d;max-width:640px">
      <h2 style="color:#1d5730">New pharmacy wholesale application</h2>
      <p><strong>${app.businessName}</strong> — ${app.fullName}</p>
      <ul>
        <li>Email: ${app.email}</li>
        <li>ABN: ${app.abn}</li>
        <li>Pharmacy reg: ${app.pharmacyReg}</li>
        <li>Compounding: ${app.compounding}</li>
...
async function notifyPharmacyApproved({ app, accessCode }) {
  ...
  const html = `
    <div style="font-family:Inter,Arial,sans-serif;color:#17211d;max-width:640px">
      ...
      <p>Questions? Reply to med@leaflock.com.au</p>
    </div>`;
  return sendMail({
    to: app.email,
...
async function sendCompliancePack({ to, pharmacyName, contactName }) {
  ...
  const business = pharmacyName || "your pharmacy";
  ...
  const html = `
    <div style="font-family:Inter,Arial,sans-serif;color:#17211d;max-width:640px;line-height:1.55">
      ...
      <ol>
        <li>Review the Stockist NDA &amp; Confidentiality Agreement, sign, and return to <a href="mailto:med@leaflock.com.au">med@leaflock.com.au</a>.</li>
  ...
  const text = [
    ...
    ...files.map((f) => `- ${f.label}`),
    "",
    "1. Review and return the signed Stockist NDA to med@leaflock.com.au",

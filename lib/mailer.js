  ...
  const html = `
    <div style="font-family:Inter,Arial,sans-serif;color:#17211d;max-width:640px">
      ...
      <ul>
        ...
        <li>ABN: ${app.abn}</li>
        ${app.pharmacyReg ? `<li>Business licence / reg: ${app.pharmacyReg}</li>` : ""}
...
async function notifyRetailStockistApproved({ app, setupToken, checkoutLink }) {
  const portalUrl = process.env.SITE_URL || "https://www.wholesale.leaflock.com.au";
  const setupUrl = passwordSetupUrl(setupToken);
  const checkoutBlock = checkoutLink
    ? `<p style="padding:14px;background:#f3eef8;border-radius:8px;margin:18px 0">
        <strong>Gummy mix email checkout</strong> — your private link (no portal login):<br>
        <a href="${checkoutLink}" style="font-weight:700;color:#1d5730;word-break:break-all">${checkoutLink}</a>
      </p>`
    : "";
  const checkoutText = checkoutLink ? `\nGummy checkout: ${checkoutLink}` : "";
  const html = `
    <div style="font-family:Inter,Arial,sans-serif;color:#17211d;max-width:640px;line-height:1.55">
      ...
      <p>Create your <strong>private password</strong> to access pricing and place orders. Each store has its own login — do not share your password.</p>
      ...
      After setup, sign in at <a href="${portalUrl}/portal.html">${portalUrl}/portal.html</a></p>
      <p>Password setup link expires in 48 hours. Your gummy checkout link stays the same unless we regenerate it. Questions? Reply to info@leaflock.com.au</p>
    </div>`;
  return sendMail({
    to: app.email,
    subject: "Your LeafLock wholesale account is approved — set your password",
    html,
    text: `Approved for ${app.businessName}. Set password: ${setupUrl}. Login: ${app.email} at ${portalUrl}/portal.html${checkoutText}`,
  });
}

async function notifyPasswordReset({ pharmacy, resetToken }) {
  const resetUrl = passwordSetupUrl(resetToken);
  const html = `
    <div style="font-family:Inter,Arial,sans-serif;color:#17211d;max-width:640px;line-height:1.55">
      <h2 style="color:#1d5730">Reset your LeafLock wholesale password</h2>
      <p>Hi,</p>
      <p>We received a request to reset the password for <strong>${pharmacy.businessName}</strong> (${pharmacy.email}).</p>
      <p style="padding:14px;background:#eaf5ed;border-radius:8px">
  ...
  return sendMail({
    ...
    html,
    text: `Reset password for ${pharmacy.businessName}: ${resetUrl}`,
  });
}
...
async function sendCompliancePack({ to, pharmacyName, contactName }) {
  ...
  const business = pharmacyName || "your retail stockist";
...
async function notifyAdminNewOrder({ order, statusLabel = "New wholesale order" }) {
  ...
  const html = `
    <div style="font-family:Inter,Arial,sans-serif;color:#17211d;max-width:640px;line-height:1.55">
      ...
      <p><strong>${order.pharmacyName || contact.businessName || "Store"}</strong></p>
      <p style="padding:14px;background:#eaf5ed;border-radius:8px">
  ...
  const text = [
    ...
    order.pharmacyName || contact.businessName,
  ...
  return sendMail({
    ...
    subject: `${statusLabel} — ${contact.businessName || order.pharmacyName} (${order.id})`,
    html,
    text,
  });
}
...
module.exports = {
  ...
  /** @deprecated */
  notifyPharmacyApproved: notifyRetailStockistApproved,
  notifyPasswordReset,
  sendCompliancePack,

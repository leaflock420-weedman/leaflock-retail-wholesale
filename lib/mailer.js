...
async function notifyRetailStockistApproved({ app, setupToken, passwordReady }) {
  ...
  return sendMail({
    ...
    html,
    text: `Approved for ${app.businessName}. Set password: ${setupUrl} Login: ${app.email} at ${portalLoginUrl} Forgot: ${forgotUrl}`,
  });
}
...
async function notifyAdminNewOrder({ order, statusLabel = "New wholesale order" }) {
  ...
  const html = `
    <div style="font-family:Inter,Arial,sans-serif;color:#17211d;max-width:640px;line-height:1.55">
      ...
      <p><strong>Attached:</strong> warehouse pick &amp; pack PDF — print or forward to fulfilment.</p>
  ...
  const text = [
    ...
    fulfillmentPdf ? "Attached: warehouse pick & pack PDF." : "",
  ]
    .filter(Boolean)
    .join("\n");
  ...
  const attachments = fulfillmentPdf
    ? [
        {
          filename: `${inv}-pick-pack.pdf`,
          content: fulfillmentPdf,
          contentType: "application/pdf",
        },
      ]
    : undefined;
  ...
  return sendMail({
    ...
    subject: `Pick & pack — ${inv} — ${contact.businessName || order.retailStockistName}`,
    html,
    text,
    attachments,
  });

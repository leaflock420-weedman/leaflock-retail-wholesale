...
async function notifyPharmacyApproved({ app, setupToken }) {
  const portalUrl = process.env.SITE_URL || "https://www.wholesale.leaflock.com.au";
  ...
  const html = `
    <div style="font-family:Inter,Arial,sans-serif;color:#17211d;max-width:640px;line-height:1.55">
      ...
      <p>This link expires in 48 hours. Questions? Reply to info@leaflock.com.au</p>
    </div>`;
  return sendMail({
    to: app.email,
    subject: "Your LeafLock wholesale account is approved — set your password",
    html,
    text: `Approved for ${app.businessName}. Set password: ${setupUrl}. Login: ${app.email} at ${portalUrl}/portal.html`,
  });
}

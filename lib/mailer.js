...
async function sendEmailTest() {
  const to = wholesaleEmail();
  const siteUrl = process.env.SITE_URL || "https://www.wholesale.leaflock.com.au";
  return sendMail({
    to,
    subject: "LeafLock Wholesale email test",
    text: `Email delivery is working for ${siteUrl}.`,
    html: `<div style="font-family:Arial,sans-serif"><h2>LeafLock Wholesale email is working</h2><p>Delivery from <a href="${siteUrl}">${siteUrl}</a> was tested successfully.</p></div>`,
  });
}

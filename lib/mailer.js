...
function setupCodeBlock(setupCode, email) {
  const manualUrl = passwordSetupUrl({ email });
  return `
    <p style="padding:16px;background:#fff8e6;border:2px solid #e6c84a;border-radius:8px;font-size:1.05rem;line-height:1.6">
      <strong>Setup code (type this if the button link fails):</strong><br>
      <span style="font-size:1.5rem;font-weight:900;letter-spacing:0.12em;color:#1d5730">${setupCode}</span><br>
      <span style="font-size:0.95rem">Go to <a href="${manualUrl}">${manualUrl}</a>, enter email <strong>${email}</strong> and this code.</span>
    </p>`;
}

async function notifyRetailStockistApproved({ app, setupToken, setupCode, passwordReady }) {
  const portalUrl = process.env.SITE_URL || "https://www.wholesale.leaflock.com.au";
  const portalLoginUrl = `${portalUrl}/portal.html`;
  const forgotUrl = `${portalUrl}/forgot-password.html`;

  if (passwordReady || !setupToken) {
    const html = `
    <div style="font-family:Inter,Arial,sans-serif;color:#17211d;max-width:640px;line-height:1.55">
      ...
      <p>Sign in with the password you chose when you applied.</p>
      ...
      Portal: <a href="${portalLoginUrl}">${portalLoginUrl}</a><br>
      Forgot password? <a href="${forgotUrl}">${forgotUrl}</a></p>
      <p>Questions? Reply to info@leaflock.com.au</p>
    </div>`;
    return sendMail({
      to: app.email,
      subject: "Your LeafLock wholesale account is approved — sign in now",
      html,
      text: [
        `Approved for ${app.businessName}.`,
  ...
  const setupUrl = passwordSetupUrl({ token: setupToken, email: app.email });
  const html = `
    <div style="font-family:Inter,Arial,sans-serif;color:#17211d;max-width:640px;line-height:1.55">
      <h2 style="color:#1d5730">LeafLock wholesale access approved</h2>
      <p>Hi ${app.fullName},</p>
      ...
      ${setupCode ? setupCodeBlock(setupCode, app.email) : ""}
      <p>Login email: <strong>${app.email}</strong><br>
      Portal: <a href="${portalLoginUrl}">${portalLoginUrl}</a><br>
  ...
  return sendMail({
    ...
    html,
    text: `Approved for ${app.businessName}. Set password: ${setupUrl}.${setupCode ? ` Setup code: ${setupCode}.` : ""} Login: ${app.email} at ${portalLoginUrl} Forgot: ${forgotUrl}`,
  });
}
...
async function notifyPasswordReset({ retailStockist, resetToken, setupCode }) {
  const resetUrl = passwordSetupUrl({ token: resetToken, email: retailStockist.email });
  const codeLine = setupCode ? `SETUP CODE: ${setupCode}` : "";
  const html = `
    <div style="font-family:Inter,Arial,sans-serif;color:#17211d;max-width:640px;line-height:1.55">
      ...
      ${setupCode ? `<p style="font-size:18px;font-weight:900;letter-spacing:0.1em;color:#1d5730;margin:16px 0">${codeLine}</p>` : ""}
      <p style="padding:14px;background:#eaf5ed;border-radius:8px">
        <a href="${resetUrl}" style="font-weight:700;color:#1d5730">Choose a new password →</a>
      </p>
      <p>Click the button — you only enter a new password (no code needed on that page).</p>
      ${setupCode ? setupCodeBlock(setupCode, retailStockist.email) : ""}
      <p>Sign in later with: <strong>${retailStockist.email}</strong></p>
      <p>If you did not request this, ignore this email. Expires in 48 hours.</p>
    </div>`;
  return sendMail({
    to: retailStockist.email,
    subject: setupCode
      ? `Reset password — code ${setupCode}`
      : "Reset your LeafLock wholesale password",
    html,
    text: [
      codeLine,
      `Reset password for ${retailStockist.businessName}.`,
      `Click: ${resetUrl}`,
      "On that page, just choose a new password.",
      setupCode
        ? `If the button fails, use code ${setupCode} at ${passwordSetupUrl({ email: retailStockist.email })}`
        : "",
      `Sign in with: ${retailStockist.email}`,
    ]
      .filter(Boolean)
      .join("\n"),
  });

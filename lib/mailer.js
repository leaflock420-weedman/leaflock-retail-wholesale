...
async function notifyPasswordReset({ retailStockist, resetToken, setupCode }) {
  ...
  const html = `
    <div style="font-family:Inter,Arial,sans-serif;color:#17211d;max-width:640px;line-height:1.55">
      ...
      <p>After saving, sign in at the portal with this exact email: <strong>${retailStockist.email}</strong></p>
      <p>If you did not request this, ignore this email. The link and code expire in 48 hours.</p>
    </div>`;
  return sendMail({
    to: retailStockist.email,
    subject: "Reset your LeafLock wholesale password",
    html,
    text: `Reset password for ${retailStockist.businessName}: ${resetUrl}${setupCode ? `\nSetup code: ${setupCode}` : ""}\nSign in with: ${retailStockist.email}`,
  });
}

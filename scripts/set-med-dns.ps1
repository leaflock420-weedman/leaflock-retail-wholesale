...
$body = @(@{ type = $recordType; name = $name; data = $value; ttl = 600 }) | ConvertTo-Json

try {
    Invoke-RestMethod -Uri "https://api.godaddy.com/v1/domains/$domain/records/$recordType/$name" -Headers $headers -Method Put -Body $body -ContentType "application/json"
    Write-Host "Updated $recordType $name.$domain -> $value" -ForegroundColor Green
} catch {
    try {
        ...
        Write-Host "Added $recordType $name.$domain -> $value" -ForegroundColor Green
    } catch {
